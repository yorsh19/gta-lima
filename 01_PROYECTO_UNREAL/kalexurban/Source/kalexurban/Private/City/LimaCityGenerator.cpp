...
void ALimaCityGenerator::GeneratePreview()
{
    ...
    UE_LOG(
        ...
        TEXT(
            ...
            "RoadSplines=%d RoadSegments=%d TerrainSnap=%s Dense=%s Spacing=%.2fm"
        ),
        GeneratedRoadSplines,
        GeneratedRoadSegments,
        (bUseStableTerrainRoads || bSnapRoadsToTerrain) ? TEXT("ON") : TEXT("OFF"),
        (bUseStableTerrainRoads || bDenseRoadTerrainSampling) ? TEXT("ON") : TEXT("OFF"),
        RoadTerrainSampleSpacingMeters
    );
}
    ...
    const FVector2D& C, float HalfW, float HalfD, float YawDeg, float SetbackMeters) const
{
    ...
    // V5.3.1: preserve configured setback, but never allow a procedural facade to touch
    // the physical asphalt edge. This is independent from visual/debug spline thickness.
    const float EffectiveClearanceMeters = FMath::Max(
        FMath::Max(RoadSafetyMarginMeters, MinimumPhysicalRoadClearanceMeters),
        StableRoadBuildingBufferMeters);
    const float SetbackUU=(FMath::Max(0.f,SetbackMeters)+EffectiveClearanceMeters)*UnitsPerMeter;
...
void ALimaCityGenerator::GenerateRoadMeshes()
{
    ...
    // V5.3.2 CRASHFIX: work from a stable snapshot. Creating/registering editor
    // components can trigger editor callbacks; never keep references into CityData
    // while doing that work.
    const TArray<FLimaRoadData> RoadsSnapshot = CityData.Roads;

    for (int32 RoadIndex = 0; RoadIndex < RoadsSnapshot.Num(); ++RoadIndex)
    {
        const FLimaRoadData Road = RoadsSnapshot[RoadIndex];
        ...
        const FName SplineName = MakeUniqueObjectName(
            ...
            *FString::Printf(TEXT("GeneratedRoadSpline_%d"), RoadIndex)
        );
        USplineComponent* Spline = NewObject<USplineComponent>(
            this, SplineName, RF_Transactional
        );
        ...
        /*
         * --------------------------------------------------------
         * ADD OSM POINTS - V5.3.2 CRASHFIX
         * --------------------------------------------------------
         * XY ALWAYS comes from OSM. We never smooth, move or reproject XY.
         * When terrain snapping is enabled we only add intermediate XY samples
         * and obtain Z from the Landscape. This prevents a long spline chord
         * from crossing through a hill or floating over a depression.
         */
        TArray<FVector> RoadSamples;
        const bool bTerrainRoad = bUseStableTerrainRoads || bSnapRoadsToTerrain;
        const bool bDenseTerrainRoad = bUseStableTerrainRoads || bDenseRoadTerrainSampling;

        if (bTerrainRoad && bDenseTerrainRoad)
        {
            const float SpacingUU = FMath::Max(50.0f, RoadTerrainSampleSpacingMeters * UnitsPerMeter);

            for (int32 I = 0; I < Road.Points.Num() - 1; ++I)
            {
                const FVector A = Road.Points[I];
                const FVector B = Road.Points[I + 1];
                const float LengthXY = FVector2D::Distance(FVector2D(A.X, A.Y), FVector2D(B.X, B.Y));
                if (!FMath::IsFinite(LengthXY) || LengthXY < 1.0f)
                {
                    continue;
                }

                // Hard cap protects the editor from malformed/huge OSM segments.
                const int32 Steps = FMath::Clamp(
                    FMath::CeilToInt(LengthXY / SpacingUU), 1, 2048
                );

                for (int32 Step = 0; Step < Steps; ++Step)
                {
                    const float Alpha = static_cast<float>(Step) / static_cast<float>(Steps);
                    const FVector Sample = FMath::Lerp(A, B, Alpha);
            ...
            RoadSamples.Add(Road.Points.Last());
        }
        else
        {
            RoadSamples = Road.Points;
        }

        if (bTerrainRoad)
        {
            // Pass 1: sample only Z. XY remains byte-for-byte derived from the OSM centerline.
            TArray<float> SampledLocalZ;
            SampledLocalZ.SetNum(RoadSamples.Num());
            for (int32 SampleIndex = 0; SampleIndex < RoadSamples.Num(); ++SampleIndex)
            {
                FVector WorldPoint = GetActorTransform().TransformPosition(RoadSamples[SampleIndex]);
                if (SnapLocationToGround(WorldPoint))
                {
                    const FVector LocalGround = GetActorTransform().InverseTransformPosition(WorldPoint);
                    SampledLocalZ[SampleIndex] = LocalGround.Z;
                }
                else
                {
                    SampledLocalZ[SampleIndex] = RoadSamples[SampleIndex].Z + RoadZOffset;
                }
            }

            // Pass 2: symmetric moving average. This removes tiny Landscape noise without
            // changing road XY or creating Bezier overshoot. End points stay anchored.
            TArray<float> SmoothedLocalZ = SampledLocalZ;
            const int32 Radius = FMath::Clamp(RoadElevationSmoothRadius, 0, 8);
            if (Radius > 0 && SampledLocalZ.Num() > 2)
            {
                for (int32 I = 1; I < SampledLocalZ.Num() - 1; ++I)
                {
                    double SumZ = 0.0;
                    int32 N = 0;
                    const int32 From = FMath::Max(0, I - Radius);
                    const int32 To = FMath::Min(SampledLocalZ.Num() - 1, I + Radius);
                    for (int32 J = From; J <= To; ++J) { SumZ += SampledLocalZ[J]; ++N; }
                    if (N > 0) SmoothedLocalZ[I] = static_cast<float>(SumZ / N);
                }
            }

            const float SurfaceOffsetUU = (bUseStableTerrainRoads ? StableRoadSurfaceOffsetMeters : RoadTerrainOffsetMeters) * UnitsPerMeter;
            for (int32 I = 0; I < RoadSamples.Num(); ++I)
            {
                FVector Point = RoadSamples[I];
                Point.Z = SmoothedLocalZ[I] + SurfaceOffsetUU;
                Spline->AddSplinePoint(Point, ESplineCoordinateSpace::Local, false);
            }
        }
        else
        {
            for (FVector Point : RoadSamples)
            {
                Point.Z += RoadZOffset;
                Spline->AddSplinePoint(Point, ESplineCoordinateSpace::Local, false);
            }
        ...
        {
            // Terrain-following roads must not overshoot between samples.
            // Linear + dense samples is deterministic and keeps the mesh on the Landscape.
            // Flat/source-Z roads keep a clamped curve for visually smooth OSM corners.
            Spline->SetSplinePointType(
                PointIndex,
                bTerrainRoad ? ESplinePointType::Linear : ESplinePointType::CurveClamped,
                false
            );
        }
        ...
        )
        {
            ...
            if (StartPosition.ContainsNaN() || EndPosition.ContainsNaN() ||
                StartTangent.ContainsNaN() || EndTangent.ContainsNaN())
            {
                UE_LOG(LogTemp, Warning, TEXT("KALEX ROAD SKIP INVALID | Road=%d Segment=%d"), RoadIndex, SegmentIndex);
                continue;
            }
            ...
            const FName SegmentName = MakeUniqueObjectName(
                ...
                *FString::Printf(TEXT("GeneratedRoadMesh_%d_%d"), RoadIndex, SegmentIndex)
            );
            USplineMeshComponent* Segment = NewObject<USplineMeshComponent>(
                this, SegmentName, RF_Transactional
            );
    ...
    UE_LOG(
        ...
        Warning,
        TEXT(
            "KALEX CITY ROADS V5.3.3 STABLE TERRAIN | "
            "Splines=%d Segments=%d TerrainSnap=%s Dense=%s Spacing=%.2fm"
        ),
        GeneratedRoadSplines,
        GeneratedRoadSegments,
        (bUseStableTerrainRoads || bSnapRoadsToTerrain) ? TEXT("ON") : TEXT("OFF"),
